...
static void cag_option_parse_access_name(cag_option_context *context,
    ...
    if (option != NULL) {
        ...
        // And now we try to parse the value. This function will also check
        // whether this option is actually supposed to have a value.
...
static void cag_option_shift(cag_option_context *context, int start, int option,
    ...
    // order of everything will be preserved. Left shift is the amount of
    // indexes the block between option and end will shift towards the start,
    // and right shift is the amount of indexes the block between start and
    // option will be shifted towards the end.
    ...
    // Let's loop through the option strings first, which we will move towards
    // the beginning.
...
void cag_option_print(const cag_option *options, size_t option_count,
                      FILE *destination) {
    cag_option_printer(options, option_count, (cag_printer)fprintf,
                       destination);
}
#endif
